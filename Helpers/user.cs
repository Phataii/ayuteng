...
public class UserHelper
{
    ...
    public async Task<Application> GetLoggedInUser(HttpRequest request)
    {
        //1. Get JWT token from cookie
        var token = request.Cookies["applicant-jwt"];
        ...
        var user = await _context.Applications.FirstOrDefaultAsync(u => u.Email == userEmail);

        return user;
    }

    public async Task<Admin> GetLoggedInAdmin(HttpRequest request)
    {
        //1. Get JWT token from cookie
        var token = request.Cookies["admin-jwt"];
        if (string.IsNullOrEmpty(token))
        {
            return null; // Redirect to login if token is missing
        }

...
using ayuteng.Models;                    // Assuming your User class is in the "rps.Models" namespace
...
public class JwtHelper
{
    ...
    private readonly string _issuer = "ayuteng"; // Set your issuer (usually your app or API name)
    private readonly string _audience = "https://ayuteng.com"; // Set the audience (who the token is intended for)

    public string GenerateJwtToken(Application user)
    {
    ...
    public string GenerateJwtToken_Admin(Admin user)
    {
        // Define the key used to sign the token
        var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_secretKey));
        var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256);
